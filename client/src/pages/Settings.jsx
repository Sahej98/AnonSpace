...
// ... (Keep existing THEMES and FONTS arrays exactly as is)
const DARK_THEMES = [
...
const LIGHT_THEMES = [
  ...
  { id: 'solarized-light', name: 'Solar. Light', colors: ['#fdf6e3', '#268bd2'], },
...
const FontCard = ({ name, value, current, onClick }) => {
  ...
  switch (value) {
    ...
    case 'opensans': fontFamily = "'Open Sans', sans-serif"; break;
    case 'lato': fontFamily = "'Lato', sans-serif"; break;
    case 'montserrat': fontFamily = "'Montserrat', sans-serif"; break;
    case 'poppins': fontFamily = "'Poppins', sans-serif"; break;
    case 'raleway': fontFamily = "'Raleway', sans-serif"; break;
    case 'nunito': fontFamily = "'Nunito', sans-serif"; break;
    case 'ubuntu': fontFamily = "'Ubuntu', sans-serif"; break;
    case 'playfair': fontFamily = "'Playfair Display', serif"; break;
    case 'lora': fontFamily = "'Lora', serif"; break;
    case 'ptserif': fontFamily = "'PT Serif', serif"; break;
    case 'robotoslab': fontFamily = "'Roboto Slab', serif"; break;
    case 'quicksand': fontFamily = "'Quicksand', sans-serif"; break;
    case 'inconsolata': fontFamily = "'Inconsolata', monospace"; break;
    case 'oswald': fontFamily = "'Oswald', sans-serif"; break;
    case 'pacifico': fontFamily = "'Pacifico', cursive"; break;
    case 'dancing': fontFamily = "'Dancing Script', cursive"; break;
    case 'vt323': fontFamily = "'VT323', monospace"; break;
    case 'pressstart': fontFamily = "'Press Start 2P', cursive"; break;
    case 'merriweather': fontFamily = "'Merriweather', serif"; break;
    case 'indie-flower': fontFamily = "'Indie Flower', cursive"; break;
    case 'fira-code': fontFamily = "'Fira Code', monospace"; break;
    case 'system': fontFamily = 'system-ui, -apple-system, sans-serif'; break;
  }
...
const Settings = () => {
  ...
  const [soundEnabled, setSoundEnabled] = useState(() => localStorage.getItem('anonSoundEnabled') !== 'false');
  ...
  const handleLogout = async () => {
      const confirmed = await dialog.confirm('Are you sure you want to sign out? You will need your User ID to log back in.', { title: 'Sign Out' });
      if (confirmed) {
          logout();
      }
  };
  ...
  const handleDeleteAccount = async () => {
      const confirmed = await dialog.confirm('Permanently delete your account and ALL your posts, comments, and data? This action CANNOT be undone.', {
    ...
    <div className='centered-page-container full-width'>
      <div
        ...
        <div className='settings-grid'>
          ...
          {/* Appearance Section */}
          <h3 className='full-span' style={{fontSize: '0.9rem', color: 'var(--text-dim)', marginTop: '1rem', fontWeight: 600, textTransform: 'uppercase'}}>Appearance</h3>

          <div className='full-span' style={{background: 'var(--input-bg)', padding: '1rem', borderRadius: '12px', border: '1px solid var(--glass-border)'}}>
            {/* Font Size Control */}
            <div style={{marginBottom: '1.5rem'}}>
                ...
                <div style={{display: 'flex', gap: '0.5rem', background: 'var(--bg-surface)', padding: '0.3rem', borderRadius: '8px', border: '1px solid var(--glass-border)'}}>
                    <button onClick={() => changeFontSize(0.85)} style={{flex: 1, padding: '0.5rem', borderRadius: '6px', background: fontSize === 0.85 ? 'var(--primary)' : 'transparent', color: fontSize === 0.85 ? 'white' : 'var(--text-muted)', fontSize: '0.8rem'}}>Small</button>
                    <button onClick={() => changeFontSize(1)} style={{flex: 1, padding: '0.5rem', borderRadius: '6px', background: fontSize === 1 ? 'var(--primary)' : 'transparent', color: fontSize === 1 ? 'white' : 'var(--text-muted)', fontSize: '0.9rem'}}>Normal</button>
                    <button onClick={() => changeFontSize(1.15)} style={{flex: 1, padding: '0.5rem', borderRadius: '6px', background: fontSize === 1.15 ? 'var(--primary)' : 'transparent', color: fontSize === 1.15 ? 'white' : 'var(--text-muted)', fontSize: '1rem'}}>Large</button>
                </div>
            </div>
            ...
            <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(80px, 1fr))', gap: '0.8rem', marginBottom: '1.5rem'}}>
              {DARK_THEMES.map((t) => (
                <ThemeCard key={t.id} name={t.name} value={t.id} colors={t.colors} current={theme} onClick={changeTheme} />
              ))}
            </div>

            <div style={{display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.8rem'}}>
              <Sun size={16} color='var(--primary)' />
              <p style={{ fontSize: '0.9rem', fontWeight: 600 }}>Light Themes</p>
            </div>
          ...
          <div className='full-span' style={{background: 'var(--input-bg)', padding: '1rem', borderRadius: '12px', border: '1px solid var(--glass-border)'}}>
            <p style={{fontSize: '0.9rem', fontWeight: 600, marginBottom: '0.8rem'}}>Typography</p>
            <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(80px, 1fr))', gap: '0.8rem'}}>
              {FONTS.map((f) => (
                <FontCard key={f.id} name={f.name} value={f.id} current={font} onClick={changeFont} />
              ))}
            </div>
          </div>

          {/* Preferences */}
          <h3 className='full-span' style={{fontSize: '0.9rem', color: 'var(--text-dim)', marginTop: '1rem', fontWeight: 600, textTransform: 'uppercase'}}>Preferences</h3>
          <SettingsItem icon={Bell} title='Notifications' subtitle='Alerts for replies' isActive={notifications} onClick={() => setNotifications(!notifications)} />
          <SettingsItem icon={Volume2} title='Sound Effects' subtitle='UI interaction sounds' isActive={soundEnabled} onClick={toggleSound} />
          <SettingsItem icon={Eye} title='Reduce Motion' subtitle='Minimize animations' isActive={reduceMotion} onClick={() => setReduceMotion(!reduceMotion)} />

          {/* Info Section */}
          <h3 className='full-span' style={{fontSize: '0.9rem', color: 'var(--text-dim)', marginTop: '1rem', fontWeight: 600, textTransform: 'uppercase'}}>Information</h3>
          
          <SettingsItem icon={FileText} title='Update Logs' subtitle='See what is new' isToggle={false} showChevron={true} onClick={() => navigate('/changelog')} />
          <SettingsItem icon={Bug} title='Report Bug / Feedback' subtitle='Help us improve' isToggle={false} showChevron={true} onClick={() => setIsFeedbackOpen(true)} />
          <SettingsItem icon={BookOpen} title='Community Rules' subtitle='Guidelines' isToggle={false} showChevron={true} onClick={() => navigate('/rules')} />
          <SettingsItem icon={Info} title='About AnonSpace' subtitle='Project info' isToggle={false} showChevron={true} onClick={() => navigate('/about')} />
          <SettingsItem className='full-span' icon={LogOut} title='Log Out' subtitle='Removes ID from device' isToggle={false} onClick={handleLogout} />

          {/* Danger Zone */}
          <h3 className='full-span' style={{fontSize: '0.9rem', color: 'var(--accent-red)', marginTop: '1rem', fontWeight: 600, textTransform: 'uppercase'}}>Danger Zone</h3>
          <SettingsItem className='full-span' icon={Trash2} title='Delete Account' subtitle='Permanently erase all data' isToggle={false} onClick={handleDeleteAccount} style={{ borderColor: 'var(--accent-red)', color: 'var(--accent-red)' }} />
        </div>
      </div>
